...
export const SessionsApi = {
   ...
   */
  getSessions(params) {
    return request('/api/sessions', { method: 'GET', params });
  },
...
3. **严格模式**：
   ...
   - 遇到未定义参数立即报错
4. **拦截器**：
   - **请求拦截器**：在发送请求前修改请求配置（URL、方法、头部、数据）
   - **响应拦截器**：在接收响应后处理响应数据
...
```json
{
  ...
  "strictMode": false,
  "requestInterceptor": "(req) => { /* 修改请求 */ return req; }",
  "responseInterceptor": "(res) => { /* 处理响应 */ return res; }",
...
- `filterUnknownParams`: 是否过滤未定义参数（默认：false）
- `strictMode`: 严格模式：拒绝未定义参数（默认：false）
- `requestInterceptor`: 请求拦截器函数（可选），用于修改请求前的数据
- `responseInterceptor`: 响应拦截器函数（可选），用于处理响应数据
...
// 使用请求拦截器
// 配置：requestInterceptor: "(req) => { req.headers['Authorization'] = 'Bearer token'; return req; }"
// 拦截器函数接收请求对象，返回修改后的请求对象

// 使用响应拦截器
// 配置：responseInterceptor: "(res) => { console.log('Response:', res); return res; }"
// 拦截器函数接收响应数据，返回处理后的数据
```
...
export const SessionsApi = {
   ...
   */
  createSession(data) {
    return request('/api/sessions', { method: 'POST', data });
  },
